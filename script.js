...
let fullGarages = [];
var enablePaws = false;
var tamiamiSwitch = false;
var arrows = {rightArrow: [], leftArrow: [], upArrow: [], downArrow: []};
...
const pawSources = {"normal": "https://parking.fiu.edu/parking-garage-counts/parking-counts-unified/sticker-paw-solid-gold.svg", 
  "error": "https://parking.fiu.edu/parking-garage-counts/parking-counts-unified/sticker-paw-blue-error.svg"};
const sensorsList = [ "PG4", "PG6" ]
const garageGroups = [["PG5", "PG4", "PG6", "Lot 1"], [ "PG1", "PG2", "Lot 5"]]; // Define garages to swap between when full
// TODO: make this into a dictionary of group name: group list pairs
const pawTrail = [{x: 0, y: 5}, {x: 0, y: 5}, {x: 1, y: 5}, {x: 1, y: 4}, {x: 2, y: 4}, {x: 2, y: 3}, {x: 3, y: 3},
  {x: 3, y: 2}, {x: 4, y: 2}, {x: 4, y: 1}, {x: 5, y: 1}, {x: 5, y: 0},{x: 5, y: 0},  // First Trail
  {x: -1, y: -1}, {x: -1, y: -1}, {x: 5, y: 5}, {x: 5, y: 4}, {x: 6, y: 4}, {x: 6, y: 3}, {x: 7, y: 3}, {x: 7, y: 2}, {x: 8, y: 2}, 
  {x: 8, y: 1}, {x: 9, y: 1}, {x: 9, y: 0}, // Second Trail
  {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, 
  {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, 
  {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, 
  {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, {x: -1, y: -1}, 
  // Pause frames
...
function drawPawGrid(type){
  const pawOverlay = document.getElementById("paw-overlay");
  const pawFragment = document.createDocumentFragment();
  for (var i = 0; i < 7; i++){
    for (var j = 0; j < 10; j++){
      const paw = document.createElement("img");
      paw.className = "paw x-" + j + " y-" + i;
      paw.setAttribute('src', pawSources["normal"]);
      pawFragment.append(paw);
      if (type == "random" && Math.trunc(Math.random() * 3) == 0){
  ...
  pawOverlay.replaceChildren(pawFragment);
}

function animatePaw(paw, x, y){
  const style = "opacity: 1";
  if (pawTrail[keyFrame].x == x && pawTrail[keyFrame].y == y){
    paw.style = style;
  }
  if (keyFrame < pawTrail.length - 1){
    if (pawTrail[keyFrame + 1].x == x && pawTrail[keyFrame + 1].y == y){
      paw.style = style;
    }
  }
}
...
function getParams(){
  ...
  if (urlParams.has('enable-paws')) {
    enablePaws = true;
  }
  if (urlParams.has('message')) {
    // setMessage(urlParams.get('message'));
    // TODO: Handle case where there is a custom message and a full message
  }
  if (urlParams.has('rarrow')) {
    arrows.rightArrow = urlParams.get('rarrow').split(",");
  }
  if (urlParams.has('tamiami')) {
    tamiamiSwitch = true;
  }
  return urlParams.has('garages') ? urlParams.get('garages').split(",") : [ "Lot 1", "PG5" ];
}
function setMessage(message){
  document.getElementById("underline-message").textContent = message;
}
function isFullMessage(){
  const countCards = document.getElementsByClassName("card");
  var isAllFull = true;
  for (const card of countCards) {
    if (card.getElementsByClassName("parking-value")[0].textContent != "Full"){
      isAllFull = false;
    }
  }
  if (isAllFull){
...
function setFullMessage(){
  setMessage("Open Spaces: Tamiami");
}
function setNormalMessage(){
  setMessage("Please Link Your Plate!");
}
...
async function fetchXML(type){
  ...
  if (type == "sensors"){
    try {
      const response = await fetch(dataSources["sensors"], requestParams)
        ...
        .then(function (buffer) {
          ...
          xmlText = text;
          sessionStorage.setItem('sensors', text);
        }); // Sensor counts data is encoded in utf-16le and needs to be converted
    } catch (error) {
      console.error("Error " + error.message);
      if (sessionStorage.getItem('sensors')) {
        xmlText = sessionStorage.getItem('sensors');
      }
    }
    ...
    try {
      ...
      sessionStorage.setItem('genetec', xmlText);
    } catch (error) {
      console.error("Error " + error.message);
      if (sessionStorage.getItem('genetec')) {
        xmlText = sessionStorage.getItem('genetec');
      }
    }
  }
...
function formatCountType(zoneName, available){
  ...
  } else if (zoneName.match("rightArrow")){
    return { "_right-arrow-template": null };
  } else {
...
function parseData(xml, garages, getFull){
  ...
  for (var item of items) { // Loop through all xml elements
    ...
    if (getFull) {
      if (available <= 20 && getCountType(zoneName) == "upper" && !fullGarages.includes(displayName) && !sensorsList.includes(displayName)){
      ...
      garages.forEach(garageID => { 
        ...
        if (displayName.match(garageID + "$") && !zoneName.includes("Total")){
          if (zoneName.match("Lvls")) {
            ...
            // Add non-split counts
            if (arrows.rightArrow.includes(displayName)) {
              occupancyList.push({ zoneName, displayName, capacity, vehicles, available: {...formatCountType(zoneName, available), ...formatCountType("rightArrow", null)} });
            } else {
              occupancyList.push({ zoneName, displayName, capacity, vehicles, available: formatCountType(zoneName, available) });
            }
          }
...
function parseDataSensors(xml, garages, getFull){
  ...
  for (var item of items) {
    ...
    const studentAvailable = { "Student": studentMax - studentSpaces <= 20 ? "Full" : studentMax - studentSpaces };
    ...
    const otherAvailable = { "Other": otherMax - otherSpaces <= 20 ? "Full" : otherMax - otherSpaces };
    if (getFull) {
...
async function main(){
  ...
  keyFrame = keyFrame == pawTrail.length - 1 ? 0 : keyFrame += 1; 
  if (tamiamiSwitch){
    isFullMessage();
  }
}
